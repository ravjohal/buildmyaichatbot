...
*   **AI Integration:** Google Gemini AI via `@google/genai` SDK for NLP, streaming API, system prompt engineering, and chunk-based retrieval. Supports model selection per chatbot with 3 options: gemini-2.5-flash (default), gemini-2.0-flash-exp, and gemini-2.5-pro. Response priority: Manual Override → Exact Cache → Semantic Cache → LLM with Chunks.
